...
class Printer {
  ...
  _catchUp(prop: string, loc: any) {
    if (!this.format.retainLines && !this.format.verbatim) return;

    // catch up to this node's newline if we're behind
    ...
    if (!this.format.verbatim) return;

    // catch up to this node's column if we're behind
    if (pos?.column != null) {
